...
{<img src="https://codeclimate.com/github/bkuhlmann/dotfile.png" alt="Code Climate GPA" />}[https://codeclimate.com/github/bkuhlmann/dotfile]
...
From the command line, the following commands are possible (please see all home_files/*.txt files for additional details):
  ...
  gld = "git log --graph --pretty=format:'%C(yellow)%H%Creset %an: %s - %Creset %C(cyan)%d%Creset %Cgreen(%cr)%Creset' --abbrev-commit --date=relative"
