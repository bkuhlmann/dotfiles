...
....
ghoste = "$EDITOR $HOME/.config/ghostty/config.ghostty"
....
...
gcam = Git Commit (all) Message - Commit changes (unstaged and staged) with message.
...
ghfi = Git History Fixup (interactive) - Select which commit to fix within current feature branch.
ghri = Git History Reword (interactive) - Select which commit to reword within current feature branch.
ghsi = Git History Split (interactive) - Select which commit to split within current feature branch.
...
hadolint_check = Haskell Dockerfile Linter - Scan Dockerfile for vulnerabilities.
